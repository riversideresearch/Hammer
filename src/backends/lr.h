...
#define H_FOREACH_(HT)                                                                             \
    {                                                                                              \
        ...
        for (size_t i__ = 0; i__ < ht__->capacity; i__++) {                                        \
            for (HHashTableEntry *hte__ = &ht__->contents[i__]; hte__; hte__ = hte__->next) {      \
                if (hte__->key == NULL)                                                            \
                    continue;

#define H_FOREACH_KEY(HT, KEYVAR)                                                                  \
    H_FOREACH_(HT)                                                                                 \
    const KEYVAR = hte__->key;

#define H_FOREACH(HT, KEYVAR, VALVAR)                                                              \
    H_FOREACH_KEY(HT, KEYVAR)                                                                      \
    VALVAR = hte__->value;
